...
std::ostream &operator<<(std::ostream &out,
  ...
  out << "glow-colour=" << params.glow_colour << '\n';
  out << "falloff=" << params.falloff << '\n';
  out << "transmittance=" << params.extinction << '\n';
  out << "intensity=" << params.intensity << '\n';

  out << "# Samples\n";
  out << "steps:0=" << params.parameters[0].primary_samples << '\n';
  out << "scattering:0=" << params.parameters[0].secondary_samples << '\n';
  out << "irradiance:0=" << params.parameters[0].irradiance_samples << '\n';

  out << "steps:1=" << params.parameters[1].primary_samples << '\n';
  out << "scattering:1=" << params.parameters[1].secondary_samples << '\n';
  out << "irradiance:1=" << params.parameters[1].irradiance_samples << '\n';

  out << "steps:2=" << params.parameters[2].primary_samples << '\n';
  out << "scattering:2=" << params.parameters[2].secondary_samples << '\n';
  out << "irradiance:2=" << params.parameters[2].irradiance_samples << '\n';
...
DepthRenderingParameters getParameters(int depth, bool irradiance = false) {
  switch (depth) {
  case 0:
    return params.parameters[0];
  case 1:
    return params.parameters[1];
  case 2:
    return params.parameters[2];
...
auto mu_a = [](float t) { return t * params.a; };

/// absorption coefficient at point t
auto mu_s = [](float t) { return t * params.s; };

/// extinction coefficient at point t
auto mu_t = [&](float t) { return mu_s(t) + mu_s(t); };

/// phase function
constexpr auto f_p = [&](Vector3D /*w*/, Vector3D /*w_bar*/) {
  return 1.0 / (4.0 * PI);
};
...
auto random_variable = []() { return random_variable_raw(); };
...
auto S2_sample = [&]() {
  thread_local std::array<Vector3D, 4000> samples;
  ...
  if (fill < samples.size()) {
    ...
    return samples[(size_t)(random_variable() * fill) % fill];
};
...
auto tau = [&](float t) { return t * params.extinction; };

// transmittance
auto T_r = [&](float t) { return std::exp(-tau(t)); };
...
RGBColor MarchingRayTracer::Li(Point3D x, Vector3D w, int depth,
                               World *world) const {
...
RGBColor MarchingRayTracer::Ls2(Point3D x, Vector3D n, int depth,
                                World *world) const {
  ...
  auto number_of_samples = getParameters(depth).irradiance_samples;
  for (int i = 0; i < number_of_samples; ++i) {
    Vector3D w_bar = S2_sample();
    result +=
        f_p(n, w_bar) * std::max(0.0, n dot w_bar) * Li(x, w_bar, depth, world);
  }
  return result / std::max(1, number_of_samples);
};

RGBColor MarchingRayTracer::Sample(Point3D x, Point3D y, float t, Vector3D w,
                                   World *world, int depth) const {
...
auto tone_map_john_hable = [](const RGBColor &colour) -> RGBColor {
  ...
  return ((colour * (A * colour + C * B) + D * E) /
          (colour * (A * colour + B) + D * F)) -
         E / F;
};

auto tone_map2 = [&](const RGBColor &colour) -> RGBColor {
  return tone_map_john_hable(colour) /
         tone_map_john_hable(RGBColor(1.5, 1.5, 1.5));
};
...
RGBColor MarchingRayTracer::trace_ray(const Ray ray, const int depth) const {
  ...
  // then subdivide that distance by some value and sample backwards along the
  // ray to accumulate light in the medium. the amount of light at a given point
  // is a function of distance from the emitter
  ...
  if (sr.hit_an_object) {
    ...
    auto surface_color = params.gather_surface && sr.material_ptr
                             ? T_r(sr.t) * sr.material_ptr->shade(sr)
                             : 0;
    auto irradiance =
        T_r(sr.t) / p(sr.t) * Ls2(sr.hit_point, sr.normal, depth, world_ptr);

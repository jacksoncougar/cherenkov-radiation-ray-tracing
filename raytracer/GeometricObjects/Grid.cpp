...
void Grid::read_ply_file(char* file_name, const int triangle_type) {
	...
	PlyProperty vert_props[] = { { "x", PLY_FLOAT, PLY_FLOAT, offsetof(Vertex,
			x), 0, 0, 0, 0 }, { "y", PLY_FLOAT, PLY_FLOAT, offsetof(Vertex, y),
			0, 0, 0, 0 }, { "z", PLY_FLOAT, PLY_FLOAT, offsetof(Vertex, z), 0,
			0, 0, 0 } };
	...
	PlyProperty face_props[] = { { "vertex_indices", PLY_INT, PLY_INT, offsetof(
			Face, verts), 1, PLY_UCHAR, PLY_UCHAR, offsetof(Face, nverts) } };
	...
	ply = ply_open_for_reading(file_name, &nelems, &elist, &file_type,
			&version);

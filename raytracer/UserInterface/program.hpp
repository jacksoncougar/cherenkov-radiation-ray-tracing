...
template <typename... Args> struct EventDelegate {
...
struct program : public nanogui::Screen {
  ...
  void save_as_png() {
    ...
    int stride = 3;
    stbi_write_png(filename.c_str(), width, height, 3, pixels.data(),
                   width * stride);
  ...
  program(const char *mesh_filename, const char *mesh2_filename) {
    ...
    renderThread =
        std::make_shared<RenderThread>(world, [&]() { onFinished(0); });
    ...
    nanogui::ref<Window> nanoguiWindow =
        gui->addWindow(Eigen::Vector2i(10, 10), "Render Settings");
    ...
    glow_color = nanogui::Color{params.glow_colour.r, params.glow_colour.g,
                                params.glow_colour.b, 1.0f};
    auto colour_widget = gui->addVariable("colour", glow_color);
    colour_widget->setFinalCallback([&](auto changed) {
      params.glow_colour =
          RGBColor{glow_color.x(), glow_color.y(), glow_color.z()};
      for (auto &&light : world->lights) {
        auto emitter = dynamic_cast<ParticleEmitter *>(light);
        if (emitter) {
    ...
    nanogui::ref<Window> nanoguiWindow3 =
        gui3->addWindow(Eigen::Vector2i(10, 10), "Light Settings");
    ...
    nanoguiWindow3->setPosition(
        Eigen::Vector2i(width - 10 - nanoguiWindow3->width(), 10));
  ...
  void render() {
    ...
    if (renderThread) {
      auto pixels = renderThread->pixel_data(false);
      glBindTexture(GL_TEXTURE_2D, texture);
      glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB8, renderThread->width,
                   renderThread->height, 0, GL_RGB, GL_UNSIGNED_BYTE,
                   pixels.data());
